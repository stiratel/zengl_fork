...
{$IFDEF WINDOWS}
  j := 0;
  for i := 0 to 15 do
    if joyGetDevCapsW( i, @joyArray[ j ].caps, SizeOf( TJOYCAPSW ) ) = 0 Then
      begin
        joyArray[ j ].id                 := i;
        joyArray[ j ].Info.Name          := utf16_GetUTF8String( joyArray[ j ].caps.szPname );
        joyArray[ j ].Info.Count.Axes    := joyArray[ j ].caps.wNumAxes;
        joyArray[ j ].Info.Count.Buttons := joyArray[ j ].caps.wNumButtons;

        caps  := @joyArray[ j ].Info.Caps;
        joyArray[ j ].axesMap[ 0 ] := JOY_AXIS_X;
        joyArray[ j ].axesMap[ 1 ] := JOY_AXIS_Y;
        axis := 2;
        if joyArray[ j ].caps.wCaps and JOYCAPS_HASZ > 0 Then
          begin
            caps^ := caps^ or JOY_HAS_Z;
            joyArray[ j ].axesMap[ axis ] := JOY_AXIS_Z;
            INC( axis );
          end;
        if joyArray[ j ].caps.wCaps and JOYCAPS_HASR > 0 Then
          begin
            caps^ := caps^ or JOY_HAS_R;
            joyArray[ j ].axesMap[ axis ] := JOY_AXIS_R;
            INC( axis );
          end;
        if joyArray[ j ].caps.wCaps and JOYCAPS_HASU > 0 Then
          begin
            caps^ := caps^ or JOY_HAS_U;
            joyArray[ j ].axesMap[ axis ] := JOY_AXIS_U;
            INC( axis );
          end;
        if joyArray[ j ].caps.wCaps and JOYCAPS_HASV > 0 Then
          begin
            caps^ := caps^ or JOY_HAS_V;
            joyArray[ j ].axesMap[ axis ] := JOY_AXIS_V;
            //INC( axis );
          end;
        if joyArray[ j ].caps.wCaps and JOYCAPS_HASPOV > 0 Then
          begin
            caps^ := caps^ or JOY_HAS_POV;
            INC( joyArray[ j ].Info.Count.Axes, 2 );
          end;

        log_Add( 'Joy: Find "' + joyArray[ j ].Info.Name + '" (ID: ' + u_IntToStr( j ) +
                 '; Axes: ' + u_IntToStr( joyArray[ j ].Info.Count.Axes ) +
                 '; Buttons: ' + u_IntToStr( joyArray[ j ].Info.Count.Buttons ) + ')' );
...
//[[[[[[[[[[[[[[[[[[[              joyArray[ i ].State.Axis[ JS_AXIS[ joyArray[ i ].axesMap[ event.number ] ] ] := Round( ( event.value / 32767 ) * 1000 ) / 1000;
            joyArray[ i ].State.Axis[ JS_AXIS[ joyArray[ i ].axesMap[ event.number ] ] ] := Round( event.value); //[[[[[[[[[[[[[[[
            end;
          JS_EVENT_BUTTON:
            case event.value of
              0:
...
{$IFDEF WINDOWS}
  ...
  for i := 0 to joyCount - 1 do
    begin
      ...
      if joyArray[ id ].caps.wCaps and JOYCAPS_POVCTS > 0 Then
        state.dwFlags := state.dwFlags or JOY_RETURNPOVCTS;

      if joyGetPosEx( id, @state ) = 0 Then
        begin
          for j := 0 to joyArray[ id ].Info.Count.Axes - 1 do
            begin
              // Say "no" to if's, and do everything trciky :)
              a     := joyArray[ id ].axesMap[ j ];
              pcaps := @joyArray[ id ].caps;
              INC( pcaps, JS_AXIS[ a ] );
              vMin  := pcaps^;
              INC( pcaps );
              vMax  := pcaps^;
              ...
              joyArray[ id ].State.Axis[ a ] := Round( ( value^ / ( vMax - vMin ) * 2 - 1 ) * 1000 ) / 1000;
            end;

          FillChar( joyArray[ id ].State.Axis[ JOY_POVX ], 8, 0 );
          if ( joyArray[ id ].Info.Caps and JOY_HAS_POV > 0 ) and ( state.dwPOV and $FFFF <> $FFFF )Then
            begin
              joyArray[ id ].State.Axis[ JOY_POVX ] := Round( m_Sin( state.dwPOV and $FFFF div 100 ) );
              joyArray[ id ].State.Axis[ JOY_POVY ] := -Round( m_Cos( state.dwPOV and $FFFF div 100 ) );
            end;

          for j := 0 to joyArray[ id ].Info.Count.Buttons - 1 do
            begin
              btn := state.wButtons and ( 1 shl j );
              if ( joyArray[ id ].State.BtnDown[ j ] ) and ( btn = 0 ) Then
                joyArray[ id ].State.BtnUp[ j ] := TRUE;

              if ( joyArray[ id ].State.BtnCanPress[ j ] ) and ( not joyArray[ id ].State.BtnDown[ j ] ) and ( btn <> 0 ) Then
                begin
                  joyArray[ id ].State.BtnPress   [ j ] := TRUE;
                  joyArray[ id ].State.BtnCanPress[ j ] := FALSE;
                end;
              joyArray[ id ].State.BtnDown[ j ] := btn <> 0;
            end;
        end;
    end;
{$ENDIF}
