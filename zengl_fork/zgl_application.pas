...
VAR                             //[[[[[[[[[[[[[[[[[[[[[[ Зта переменная нужна чтобы выводить движком
ZGL_BEZ_VIDEO: BOOLEAN=FALSE;   //[[[[[[[[[[[[[[[[[[[[[[ один звук, с видеокном размером 0x0 пикселей.
                                //Закрытие движка при ZGL_BEZ_VIDEO=TRUE будет немножко с сегфолтом,
                                //потому что оно не оптимизировано, только открытие оптимизировано.
ZGL_PAUZA: BOOLEAN=FALSE;       //[[[[[[[[[[[[[[ В ПРИНЦИПЕ ПРИ =TRUE ПАУЗА, НО ПАУЗИТ НЕ ВСЁ ЧТО НАДО
  ...
  case Msg of
      ...
      begin
        if not appAutoPause Then
         // appTimer := SetTimer( wndHandle, 1, 1, nil ); //[[[[[[[[[[[[[[[[[[[[
          if  ZGL_PAUZA= false then appTimer := SetTimer( wndHandle, 1, 1, nil );  //[[[[[[[[[[[[[                
      end;
    WM_EXITSIZEMOVE:
      begin
        if appTimer > 0 Then
    ...
    WM_MOVING:
      begin
        ...
        if ZGL_PAUZA then timer_Reset(); //[[[[[[[[[[[[[
        if ZGL_PAUZA=false then if appAutoPause Then
...
{$IFDEF MACOSX}
  ...
  if ZGL_PAUZA Then appPause := TRUE; //[[[[[[[[[[[[[[[[
  if appWork Then
  case eClass of
    kEventClassCommand:
      case eKind of
...
begin
  ...
  if if ZGL_PAUZA Then appPause := TRUE; //[[[[[[[[[[[[[
  if appAutoPause Then appPause := TRUE;
  if appWork and Assigned( app_PActivate ) Then
    app_PActivate( FALSE );
